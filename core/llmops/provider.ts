...
async function request(path: string, body: unknown, baseUrl = config.LLM_BASE_URL, apiKey = config.LLM_API_KEY, timeoutMs = 60000) {
...
async function anthropicRequest(messages: Message[], maxTokens: number) {
  ...
  const response = await fetch(config.ANTHROPIC_BASE_URL.replace(/\/$/, '') + '/v1/messages', {
    ...
    body: JSON.stringify({ model: config.LLM_MODEL, system, messages: messages.filter(message => message.role !== 'system'), temperature: 0, max_tokens: maxTokens }),
    signal: AbortSignal.timeout(60000), redirect: 'error'
  });
  if (!response.ok) throw new Error('Provedor de IA retornou HTTP ' + response.status + '. Verifique a configuração no servidor.');
  return response.json() as Promise<any>;
}
export async function generate(messages: Message[], maxTokens = 3000) {
  if (!generationEnabled()) throw new Error('Modelo de geração não configurado.');
  const result = config.LLM_PROVIDER === 'anthropic' ? await anthropicRequest(messages, maxTokens) : await request('/chat/completions', {
    model: config.LLM_MODEL, messages, temperature: 0,
...
export async function embed(texts: string[], options: { attempts?: number; timeoutMs?: number } = {}): Promise<number[][]> {
  ...
  for (let attempt = 1; ; attempt++) {
    try { result = await request('/embeddings', { model: config.EMBEDDING_MODEL, input: texts }, baseUrl, apiKey, timeoutMs); break; }
