...
export async function plan(question: string, agent: boolean, documentNames: string[] = []) {
  ...
  const result = await generate([
    ...
    { role: 'user', content: JSON.stringify({ question, availableDocuments: documentNames.slice(0, 100) }) }
  ], 800);
