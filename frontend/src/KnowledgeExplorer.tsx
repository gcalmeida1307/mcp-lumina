...
import { api } from './api';
import type { DocumentPassages, DomainGraph, NeuralMap, MapRelation } from '../../core/knowledge/types';
...
export function NeuralExplorer({ data, error, onRetry, iconMap, onOpen, canReindex = false }: { data?: NeuralMap; error: string; onRetry: () => void; iconMap: Record<string, LucideIcon>; onOpen: (id: string) => void; canReindex?: boolean }) {
  ...
  const [selectedRelation, setSelectedRelation] = useState<MapRelation>();
  ...
  const [knowledge, setKnowledge] = useState<{ enabled: boolean; pausedForChat: boolean; jobs: Record<string, number>; relations: Record<string, number> }>();
  ...
  useEffect(() => {
    ...
    void api<NonNullable<typeof knowledge>>(`/knowledge/status?domain=${encodeURIComponent(location.domain)}`, { signal: controller.signal })
      .then(result => { if (!controller.signal.aborted) setKnowledge(result); }).catch(() => {});
    return () => controller.abort();
  }, [location.domain, reload]);
  useEffect(() => {
    if (!location.domain) return;
    const controller = new AbortController();
  ...
  const graph = useMemo(() => {
    ...
    if (!location.domain) {
      ...
      if (showRelations) detail.relations.forEach(item => edges.push({ id: `relation-${item.source}-${item.target}`, source: item.source, target: item.target, type: 'bezier', label: item.validation === 'validated' ? 'Trecho confirmado' : `${Math.round(item.score * 100)}% · candidata`, data: item, ariaLabel: `${item.method === 'semantic' ? 'Similaridade vetorial' : 'Termos em comum'}: ${Math.round(item.score * 100)} por cento`, style: { stroke: item.method === 'semantic' ? '#74d9c1' : '#d3ad70', strokeWidth: 1.5, strokeDasharray: item.method === 'lexical' ? '5 5' : undefined }, labelStyle: { fill: '#c9d7ed', fontSize: 10 }, labelBgStyle: { fill: '#18243b' } }));
    }
    return { nodes, edges };
  ...
  return <section className={`kg-explorer${expanded ? ' kg-expanded' : ''} kg-constellation${!showInspector ? ' kg-panel-hidden' : ''}`} aria-label="Explorador de conhecimento" onKeyDown={event => { if (event.key === 'Escape') { if (expanded) setExpanded(false); else setSelected(undefined); } }}>
    ...
    <aside ref={panel} id="knowledge-inspector" hidden={!showInspector} className="kg-inspector" aria-label="Detalhes e fonte"><span className="kg-kicker">{selectedPassage ? 'FONTE ORIGINAL' : location.document ? 'DOCUMENTO' : domain ? 'MÓDULO DE CONHECIMENTO' : 'COMECE A EXPLORAR'}</span><div className="kg-detail-icon" style={{ color: domain?.color }}>{location.document ? <FileText size={25} /> : domain ? <Boxes size={25} /> : <BrainCircuit size={25} />}</div><h2>{selectedPassage ? `Trecho ${selectedPassage.index + 1}` : title}</h2>
      {selectedRelation?.evidence && <div className="kg-relation-note"><h3>Trecho confirmado nas duas fontes</h3><blockquote>{selectedRelation.evidence.quote}</blockquote><small>Verificação: {selectedRelation.evidence.validator}. Confirma presença textual nas fontes.</small><div>{[selectedRelation.source, selectedRelation.target].map((id, index) => <button key={id} className="button secondary" onClick={() => go({ domain: location.domain, document: id, title: detail?.documents.find(item => item.id === id)?.name ?? `Fonte ${index + 1}` })}>Abrir fonte {index + 1}</button>)}</div></div>}
      ...
      {selectedPassage ? <><p className="kg-detail-caption">{location.title}</p><span className="kg-badge">{selectedPassage.embedded ? 'Representação vetorial disponível' : 'Disponível para busca textual'}</span><div className="kg-source-text">{selectedPassage.text}</div><button className="button secondary" onClick={() => setSelected(undefined)}>Voltar ao documento</button></> : <>
        ...
        {domain && knowledge && <div className="kg-embedding-info"><span className="kg-kicker">CONHECIMENTO EM SEGUNDO PLANO</span><strong>{!knowledge.enabled ? 'Processamento desativado' : knowledge.pausedForChat ? 'Pausado para priorizar o chat' : 'Processamento incremental'}</strong><p>{count((knowledge.jobs.pending ?? 0) + (knowledge.jobs.retry ?? 0))} na fila · {count(knowledge.jobs.running ?? 0)} em execução · {count(knowledge.jobs.failed ?? 0)} com falha</p><small>{count(knowledge.relations.validated ?? 0)} relações com trecho confirmado · {count(knowledge.relations.candidate ?? 0)} candidatas. Use Atualizar mapa para acompanhar.</small></div>}
        ...
        {location.domain && !location.document && <div className="kg-relation-note"><h3><Network size={16} /> Sobre as relações</h3><p>As relações são preparadas em segundo plano. Similaridade indica uma conexão candidata. “Trecho confirmado” significa que a mesma afirmação foi localizada nas duas fontes; não comprova sua veracidade ou aplicabilidade.</p><small>Candidatos limitados por documento, com prioridade para o chat. As linhas mostram relações já processadas entre os nós visíveis.</small></div>}
