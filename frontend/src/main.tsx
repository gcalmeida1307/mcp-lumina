...
type Page = 'overview' | 'chat' | 'knowledge' | 'pipeline' | 'domains' | 'integrations' | 'observability' | 'neural' | 'metrics' | 'settings';
...
const titles: Record<Page, string> = { overview: 'Visão geral', chat: 'Chat institucional', knowledge: 'Base de conhecimento', pipeline: 'Pipeline Explorer', domains: 'Módulos de domínio', integrations: 'Integrações', observability: 'Observabilidade', neural: 'Mapa Neural', metrics: 'Métricas', settings: 'Configurações' };
const nav = [
  ...
  { id: 'observability', icon: Activity }, { id: 'neural', icon: BrainCircuit }, { id: 'metrics', icon: ChartNoAxesCombined }
] as const;
...
function App() {
  ...
  useEffect(() => {
    ...
    if (page === 'neural' || page === 'metrics') loadNeuralMap();
    if (page === 'metrics' && domains.length) void Promise.all(domains.map(item => api<Run[]>('/runs?domain=' + encodeURIComponent(item.id)))).then(results => setMetricsRuns(results.flat())).catch(e => setError(e.message));
  }, [page, isAdmin, domains]);
  useEffect(() => {
  ...
  return <div className="app">
    ...
    <div className="shell">
      ...
      <main>
        ...
        {page === 'metrics' && <MetricsDashboard runs={metricsRuns} domains={domains} neuralMap={neuralMap} />}
